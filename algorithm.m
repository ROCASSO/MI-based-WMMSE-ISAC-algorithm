    ...
    for l = 1:C
      for s = 1:s_path+1
        ...
        AOA = AOA2*pi;
            ...
            strvctc2 = zeros(d,1);
            for t = 1:T
                strvctc1(t) = exp(1i * (2*pi/28) * (t-1) * 2 * sin(AOA));%28GHz

            end
            for c = 1:d
                strvctc2(c) = exp(1i * (2*pi/28) * (c-1) * 2 * sin(AOD));
            end
...
for j = I+1 : I+S
     ...
     for n = 1:L+1
        ...
        strvct2 = zeros(Nr,1);
        for t = 1:T
            strvct1(t) = exp(1i * (2*pi/28) * (t-1) * 2 * sin(LAODS(n)));
        end
        for s = 1:Nr
            strvct2(s) = exp(1i * (2*pi/28) * (s-1) * 2 * sin(LAOAS(n)));
        end
